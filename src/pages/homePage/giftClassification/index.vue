<template>
  <div class="giftClassification-page">
    ...
    <RightAnimation :maskShow="maskShow" @mask="maskShow = false">
      ...
      <van-pull-refresh v-model="isLoading"
        ...
        <div class="new-products">
          <GoodsList v-if="isShowGoodList" :goodsList="giftGoodsList"></GoodsList>
        </div>
      </van-list>
    </van-pull-refresh>
  </div>
...
export default {
  ...
  methods: {
    init() {
      ...
      .then(res => {
        if(res.data.code == '0') {
          ...
          this.giftGoodsList = this.giftGoodsList.concat(res.data.info.list)
    ...
    search() {
      ...
      let giftHistoryList = localStorage.getItem('giftHistoryList').split(',')
      giftHistoryList.unshift(this.keyword)
      giftHistoryList = Array.from(new Set(giftHistoryList))
      localStorage.setItem('giftHistoryList', giftHistoryList)
    },
    tabToggle(index) {
    ...
    onRefresh() {
      setTimeout(() => {
        if(this.giftGoodsList.length == 0) {
          this.isLoading = false
          this.loading = false
        }else {
          this.loading = true
          ...
          .then(res => {
            if(res.data.code == '0') {
              ...
              this.loading = false
              this.giftGoodsList = []
              this.giftGoodsList = res.data.info.list
              this.total = Math.ceil(Number(res.data.info.total) / Number(this.size))
              this.page = Number(this.page) + 1;
            }
          })
