<template>
  <div class="giftExclusive-page">
    <div class="search-top">
      <div class="search-box">
        <router-link :to="{ name: 'searchHistory', query: {giftType: true} }">
          <img src="@/assets/homePage/srerch_icon.png" alt="">
          <input type="text" placeholder="请输入商品名称或者关键词" readonly>
        </router-link>
      </div>
    </div>
      ...
      <van-pull-refresh
        ...
        >
          <div class="area">
            ...
            <GoodsList v-if="isShowGoodList" :goodsList="giftList"></GoodsList>
          </div>
        </van-list>
      </van-pull-refresh>
    </div>
...
import { Tab, Tabs, PullRefresh, List, Loading } from 'vant';
Vue.use(Tab).use(Tabs).use(PullRefresh).use(List).use(Loading);
export default {
  name: 'giftExclusive',
  data() {
    return {
    ...
    init() {
      ...
      .then(res => {
        if(res.data.code === '0') {
          ...
          this.giftList = this.giftList.concat(res.data.info.list)
          // 加载状态结束
          setTimeout(() => {
            this.loading = false
        ...
        if(this.giftList.length == 0) {
          ...
          .then(res => {
            if(res.data.code == '0') {
              ...
              this.giftList = [];
              this.giftList = res.data.info.list;
              this.isShowGoodList = true;
              this.total = Math.ceil(Number(res.data.info.total) / Number(this.size))
              if (this.giftList.length === 0) {
                this.noOrder = true;
